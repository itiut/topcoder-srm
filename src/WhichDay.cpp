...
public:
    string getDay(vector <string> notOnThisDay) {
        ...
        for (auto day : weekdays) {
            if (find(begin(notOnThisDay), end(notOnThisDay), day) == end(notOnThisDay)) {
                return day;
            }
        }
        return "";
...
namespace moj_harness {
	...
	int run_test_case(int casenum__) {
		...
		case 0: {
			...
			string received__         = WhichDay().getDay(vector <string>(notOnThisDay, notOnThisDay + (sizeof notOnThisDay / sizeof notOnThisDay[0])));
