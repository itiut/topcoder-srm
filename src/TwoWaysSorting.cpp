...
public:
    string sortingMethod(vector <string> stringList) {
        ...
        vector<bool> isSorted(2, true);
        for (int i = 0; i < n - 1; i++) {
            isSorted[0] = isSorted[0] && (stringList[i] <= stringList[i + 1]);
            isSorted[1] = isSorted[1] && (stringList[i].size() <= stringList[i + 1].size());
        }
        if (isSorted[0] && isSorted[1]) {
            return "both";
        }
        if (isSorted[0]) {
